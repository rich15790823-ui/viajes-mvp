const NV = {
  ...
  choose(elInput, listEl, item){
    elInput.value = item.code || item.id || item.name;
  ...
  bindAutocomplete(elInput, listEl){
    ...
    document.addEventListener("click", (e)=>{
      if(!listEl.contains(e.target) && e.target!==elInput){
        listEl.classList.remove("open");
      }
    });
  },
  parseRoute(raw){
    // Acepta 'MEX CUN', 'MID-MTY', 'mex   cun'...
...
window.addEventListener("DOMContentLoaded", ()=>{
  ...
  if(r){
    r.addEventListener("change", ()=>{
      const parsed = NV.parseRoute(r.value);
      if(parsed){
        if(o) o.value = parsed.from;
        if(d) d.value = parsed.to;
      }
    });
  }
});
